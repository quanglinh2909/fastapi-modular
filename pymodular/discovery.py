...
from pymodular.core.controller import build_router, controllers_in
from pymodular.core.logging import get_logger
from pymodular.core.websocket import build_ws_router, gateways_in
...
Khớp với bộ khung `pym init` sinh ra: `src/api/<module>/`. Dự án xếp khác thì
...
def _package_dir(package: str) -> Path:
    ...
    if not paths:
        raise RuntimeError(
            ...
            "Đây là nơi chứa các module ứng dụng — xem pymodular.discovery."
        )
    return Path(paths[0])
