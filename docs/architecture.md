...
│   ├── connection.py          @<hạ tầng>_on_connect / _on_disconnect (dùng chung mọi hạ tầng)
...
│   ├── database/              base · memory · sql · mongo · factory · repository · query (builder) · subscribers
...
fam test       # 1216 test trên backend memory (432 test nữa cần hạ tầng hoặc driver thật)
```
