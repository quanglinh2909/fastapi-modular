...
| `repo.createQueryBuilder()` (TypeORM) | `repo.query()` — JOIN (cả self join, FULL), so sánh, NULL, `group_by`/`having`; xem [database.md](database.md#truy-vấn-phức-tạp--join-lớnbé-null) |
...
fam test       # 962 test trên backend memory (132 test nữa cần hạ tầng hoặc driver thật)
```
