...
Vấn đề: sổ kết nối nằm trong BỘ NHỚ của một tiến trình. `fam run --workers 4` chạy 4
...
- `redis`: dùng Redis pub/sub. Bật bằng `APP_WS__ADAPTER=redis`, cài bằng
  `pip install 'fastapi-modular[redis]'`. Thư viện redis chỉ được import khi thật sự chọn nó.
"""
    ...
    async def start(self, on_message: OnMessage) -> None:
        try:
            ...
            raise ComponentNotEnabledError(
                ...
                "Chạy `pip install 'fastapi-modular[redis]'`, hoặc đổi về APP_WS__ADAPTER=local nếu "
                "chỉ chạy một worker."
            ) from exc
