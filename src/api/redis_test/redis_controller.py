...
class RedisTestController:
    ...
    @get("/bao-cao/{code}", summary="Việc chậm 0.4s, có cache 30 giây")
    async def bao_cao(self, code: str) -> dict[str, object]:
        bat_dau = time.perf_counter()
        du_lieu = await self._bao_cao.bao_cao(code)
        ...
        return {
            ...
            "tu_cache": mili < 100,
            "con_song": await self._redis.ttl(f"bao-cao:{code}"),
        }
    ...
    @post("/dem/{counter}", summary="Đếm nguyên tử, cửa sổ 60 giây")
    async def dem(self, counter: str) -> dict[str, object]:
        ...
        count = await self._redis.incr(f"dem:{counter}", ttl=60)
        return {"ten": counter, "so": count, "con_song": await self._redis.ttl(f"dem:{counter}")}

    @post("/phat", summary="Phát tin lên kênh — mọi worker đang nghe đều nhận")
    async def phat(self, payload: GiaMoi) -> dict[str, object]:
        listeners = await self._redis.publish(KENH, payload.model_dump())
        return {
            "kenh": KENH,
            "nguoi_nghe": listeners,
            # 0 nghĩa là lúc này KHÔNG AI nghe và tin vừa rồi mất luôn —
            # pub/sub của Redis không lưu lại gì.
            "canh_bao": None if listeners else "không ai đang nghe, tin đã mất",
        }
